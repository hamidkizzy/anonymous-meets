...
function renderMessage(msg, { animate = true } = {}) {
  ...
  if (msg.reply_to_id) {
    ...
    const authorLabel = msg.reply_sender_id === session.user.id ? "You" : tagFor(msg.reply_sender_id).label;
    quote.innerHTML = `
...
function startReply(msg) {
  ...
  const authorLabel = msg.sender_id === session.user.id ? "You" : tagFor(msg.sender_id).label;
...
async function sendMessage() {
  ...
  const payload = { group_id: group.id, sender_id: session.user.id, content };
  if (replyingTo) {
